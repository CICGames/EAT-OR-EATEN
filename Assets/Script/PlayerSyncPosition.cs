...
public class PlayerSyncPosition : NetworkBehaviour {
    ...
    [SyncVar] private Quaternion _syncAimRotation = Quaternion.identity;
    ...
    private void LerpMove() {
        if (!isLocalPlayer) { // 플레이어 자신이 아닌 다른 플레이어들 움직임
            ...
            _aimTransform.rotation = Quaternion.Lerp(_aimTransform.rotation, _syncAimRotation, Time.deltaTime * _lerpRate);
        }
    }

    [Command] // 서버에서 수행
    void CmdProvidePostionToServer(Vector3 _pos, Quaternion _rot, Vector3 _scale, Quaternion _aimRotation) {
        _syncPosition = _pos;
        _syncRatation = _rot;
        _syncScale = _scale;
        _syncAimRotation = _aimRotation;
    }

    [ClientCallback] // 클라이언트에서만 동장
    private void TransMoveInformation() {
        if (isLocalPlayer) {
            CmdProvidePostionToServer(_myTransfrom.position, _myTransfrom.rotation, _myTransfrom.localScale, _aimTransform.rotation);
        }
    }
}
